...
import {Wallet, EventTargetImpl, helper, anumacore, CONFIRMATION_COUNT, COINBASE_CFM_COUNT} from '@anuma-network/wallet';
const {HDPrivateKey} = anumacore;
...
export const initAnumaFramework = (opt:{workerPath?:string}={})=>{
	return new Promise<void>((resolve, reject)=>{
		helper.dpc(2000, ()=>{
			...
			if(IS_NODE_CLI){
				...
				let {
					workerPath="/node_modules/@anuma-network/wallet-worker/worker.js"
				} = opt
				url = new URL(workerPath, baseURL);
			}
			workerLog.info("initAnumaFramework", url, baseURL)
...
class WalletWrapper extends EventTargetImpl{
	...
	initWorker(){
		if(!worker)
			throw new Error("Please init anuma framework using 'await initAnumaFramework();'.")
		this.worker = worker;
	 ...
	 * @param txParams
	 * @param txParams.toAddr To address in cashaddr format (e.g. anumatest:qq0d6h0prjm5mpdld5pncst3adu0yam6xch4tr69k2)
	 * @param txParams.amount Amount to send in sompis (100000000 (1e8) sompis in 1 ANUM)
